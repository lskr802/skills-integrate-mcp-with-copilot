document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateAuthUI() {
    ...
    loginIconBtn.classList.toggle("hidden", isLoggedIn);
    authUsernameSpan.textContent = isLoggedIn
      ? `Logged in as ${authToken.username}`
      : "";
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        authToken = { token: result.token, username: result.username };
        loginModal.classList.add("hidden");
        updateAuthUI();
      } else {
  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Show delete buttons only when logged in
